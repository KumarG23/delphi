...
export function AccountActivityPanel({ accountId }: { accountId: string }) {
  ...
  async function handleSync() {
    ...
    try {
      ...
      const warning = result.results.find((item) => item.balance_warning)?.balance_warning;
      const transactionMessage = changes === 0
      ...
      const balanceMessage = result.balances_refreshed > 0
        ? ` ${result.balances_refreshed === 1 ? 'Balance' : 'Balances'} refreshed.`
        : warning
          ? ' Transactions synced, but the balance refresh needs another try.'
          : '';

      await infoDialog('Account synced', `${transactionMessage}${balanceMessage}`);
    } catch (error) {
        ...
        <View style={styles.plaidCard}>
          <View style={styles.plaidCopy}>
            ...
            <Text style={styles.plaidMeta}>
              {plaidStatus.institution_name || 'Connected institution'}
            </Text>
            <Text style={styles.plaidFreshness}>
              Activity {relativeAge(plaidStatus.last_synced_at)} · Balance {relativeAge(plaidStatus.last_balance_at)}
            </Text>
          </View>
          <Pressable
            onPress={handleSync}
